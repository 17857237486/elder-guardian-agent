...
class P0VitalCooldown:
    ...
    @staticmethod
    def should_cooldown(event: NormalizedEventV2) -> bool:
        return _risk_text(event.risk_level) == "P0" and str(event.event_type) == EventType.SPO2_LOW.value

    @staticmethod
    def dedupe_key(event: NormalizedEventV2) -> str:
        return f"{event.elder_id}:{event.event_type}:P0"
